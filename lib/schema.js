...
var Field = (function () {
    ...
    Field.prototype.isIdField = function () {
        return Schema.idSuffix != null && Schema.idSuffix.length && this.fieldName.length > Schema.idSuffix.length && this.fieldName.substr(this.fieldName.length - Schema.idSuffix.length, Schema.idSuffix.length) == Schema.idSuffix;
    };
...
function read(database, username, password, options, callback) {
    ...
    function processReferences(err, rows) {
        ...
        if (rows == null) {
            var err = new Error("No references returned for database.");
            callback(err, null);
            return;
        }

        if (rows.length == 0) {
            var err = new Error("Empty references returned for database.");
            callback(err, null);
            return;
        }
