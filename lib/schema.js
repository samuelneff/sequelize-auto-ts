...
var Field = (function () {
    ...
    Field.prototype.translatedFieldType = function () {
        ...
        if (translated == undefined) {
            ...
            if (fieldTypeLength < 6 || (fieldType.substr(fieldTypeLength - 4, 4) !== 'Pojo' && fieldType.substr(fieldTypeLength - 6, 6) !== 'Pojo[]')) {
    ...
    Field.prototype.customFieldType = function () {
        return this.isIdField() ? (this.targetIdFieldType == undefined ? this.fieldNameProperCase() : this.targetIdFieldType) : this.isReference ? this.fieldType : this.translatedFieldType();
    };
...
function read(database, username, password, options, callback) {
    ...
    function readCustomFields(originalRows) {
        ...
        var sql = "select table_name, column_name, data_type, referenced_table_name, referenced_column_name, ordinal_position " + "from SequelizeCustomFieldDefinitions " + "order by table_name, ordinal_position";
    ...
    function processReferences(err, rows) {
        ...
        function processReferenceRow(row) {
            ...
            childTable.fields.push(new Field(util.camelCase(Sequelize.Utils.singularize(associationName === undefined ? row.referenced_table_name : associationName)), Sequelize.Utils.singularize(row.referenced_table_name) + 'Pojo', childTable, true));
    ...
    function processIdFields() {
        ...
        for (var tableIndex = 0; tableIndex < schema.tables.length; tableIndex++) {
            ...
            if (!idFieldLookup[fieldName] && fieldName.length > idSuffixLen && fieldName.substr(fieldName.length - idSuffixLen, idSuffixLen).toLocaleLowerCase() == idSuffix) {
                idFields.push(field);
